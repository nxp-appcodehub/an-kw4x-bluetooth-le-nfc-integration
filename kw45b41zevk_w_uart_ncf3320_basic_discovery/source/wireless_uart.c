...
extern void NFC_BasicDiscoveryLoop_Start (void);
/************************************************************************************
 *************************************************************************************
 * Private macros
 *************************************************************************************
...
#if gUseNfcLibrary_d
nfcCardInfo_t gNfcCardInfo = {0x00};
#endif
/* Timer Callbacks */
#if gWuart_CentralRole_c == 1
...
)
{
    switch(pNotificationEvent->eventType)
    {
        case gNotifEventNone_c:
        {
            ...
            Serial_PrintDec(pNotificationEvent->status);
            Serial_Print("\n\r", gAllowToBlock_d);
            break;
        }
...
static void BleApp_SendNfcCardInfoHandler(void *pParam)
{
	...
	if(dataToSend!=1)
	{
		 (void)App_PostCallbackMessage((appCallbackHandler_t)BleApp_SendNfcCardInfo, NULL);
	}
}
