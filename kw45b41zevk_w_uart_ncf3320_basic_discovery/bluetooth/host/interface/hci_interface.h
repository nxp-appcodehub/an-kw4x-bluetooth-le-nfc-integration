...
#if (gBLE51_d == TRUE)
#if (gBLE51_ConnectionlessCTESupport_d == TRUE)
...
#if (gBLE51_ConnectionCTESupport_d == TRUE)
...
#if (gBLE51_PeriodicAdvSyncTransferSupport_d == TRUE)
...
#if (gBLE_HADM_d == TRUE)
typedef struct hciLeHadmEvent_tag
{
    uint8_t                                 *pData;
} hciLeHadmEvent_t;
#endif
...
typedef struct hciLeCsReadRemoteSupportedCapabilitiesEvent_tag
{
    ...
    uint16_t        maxProceduresSupported;
    uint8_t         numAntennasSupported;
    uint8_t         rolesSupported;
    uint8_t         modesSupported;
    uint8_t         RTTCapability;
    uint8_t         RTTCoarseN;
    uint8_t         RTTSoundingN;
    uint8_t         RTTRandomPayload;
    uint8_t         RTTPhysSupported;
    uint16_t        subfeaturesSupported;
    uint16_t        TIP1timesSupported;
    uint16_t        TIP2timesSupported;
    uint16_t        TFCStimesSupported;
    uint16_t        TPMtimesSupported;
...
typedef struct hciLeCsProcedureEnableCompleteEvent_tag
{
    ...
    uint8_t         subeventInterval[gCSMaxSubeventLen_c];
...
#if ((gBLE51_ConnectionCTESupport_d == TRUE) || (gBLE51_ConnectionlessCTESupport_d == TRUE))
...
typedef struct hciLeCsReadLocalFAETable_tag
{
    hciErrorCode_t  status;
    uint8_t         aFAETable[gCSReflectorTableSize_c];
} hciLeCsReadLocalFAETable_t;
...
typedef struct hciLeCsSetAFH_tag
{
    hciErrorCode_t  status;
} hciLeCsSetAFH_t;
...
typedef struct hciVendorHandoverGetLlContextCommandComplete_tag
{
    ...
    uint8_t         llContext[gHciVendorHandoverMaxLlContextSize_c];
...
#endif /* gBLE_ChannelSounding_d */
#if (gBLE_HADM_d == TRUE)
        hciLeHadmEvent_t                                hciLeHadmEvent;
#endif /* gBLE_HADM_d */
...
typedef struct
{
    ...
    union
    {
        ...
        hciLeCsReadLocalFAETable_t                                hciLeCsReadLocalFAETable;
        hciLeCsWriteRemoteFAETable_t                              hciLeCsWriteRemoteFAETable;
        hciLeCsSetAFH_t                                           hciLeCsSetAFH;
...
#if gConnCentralSupported_d || (gLeBroadcasterSupported_d && gBLE50_d && gBLE51_d)
/*! *********************************************************************************
...
/*! *********************************************************************************
* \fn           bleResult_t Hci_VendorGetConnectionParameters(uint16_t connectionHandle)
*
...
*
********************************************************************************** */
bleResult_t Hci_VendorGetConnectionParameters(uint16_t connectionHandle);
